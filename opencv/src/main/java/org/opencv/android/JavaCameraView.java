...
public class JavaCameraView extends CameraBridgeViewBase implements PreviewCallback
{
        ...
        synchronized (this) {
            ...
            if (mCameraIndex == CAMERA_ID_ANY) {
                ...
                if (camera == null && Build.VERSION.SDK_INT >= Build.VERSION_CODES.GINGERBREAD) {
                    ...
                    for (int camIdx = 0; camIdx < Camera.getNumberOfCameras(); ++camIdx) {
                        Log.d(TAG, "Trying to open camera with new open(" + Integer.valueOf
                                (camIdx) + ")");
                        try {
                            camera = Camera.open(camIdx);
                            connected = true;
                        } catch (RuntimeException e) {
                            Log.e(TAG, "Camera #" + camIdx + "failed to open: " + e
                                    .getLocalizedMessage());
                        }
                        if (connected) break;
                    }
            ...
            try {
                ...
                if (sizes != null) {
                    ...
                    frameChain[0] = new Mat(mFrameHeight + (mFrameHeight / 2), mFrameWidth,
                            CvType.CV_8UC1);
                    frameChain[1] = new Mat(mFrameHeight + (mFrameHeight / 2), mFrameWidth,
                            CvType.CV_8UC1);

                    AllocateCache();

                    cameraFrame = new JavaCameraFrame[2];
                    cameraFrame[0] = new JavaCameraFrame(frameChain[0], mFrameWidth, mFrameHeight);
                    cameraFrame[1] = new JavaCameraFrame(frameChain[1], mFrameWidth, mFrameHeight);
    ...
    protected void releaseCamera()
    {
        synchronized (this) {
            ...
            if (cameraFrame != null) {
                cameraFrame[0].release();
                cameraFrame[1].release();
            }
        }
    }
    ...
    public void onPreviewFrame(byte[] frame, Camera arg1)
    {
        ...
        synchronized (this) {
            frameChain[chainIdx].put(0, 0, frame);
        ...
        public void run()
        {
            do {
                ...
                if (!stopThread && hasFrame) {
                    if (!frameChain[1 - chainIdx].empty())
